...
if prompt := st.chat_input("Ask me something...", accept_file="multiple", file_type=["png", "jpg", "jpeg"]):
    # Store user message
    with st.chat_message("user", avatar=":material/flare:"):
        if prompt.text:
    ...
    with st.chat_message("assistant", avatar=":material/moon_stars:"):
        with st.empty():
            full_response = ""
            ...
            full_response += first_chunk["message"]["content"] if first_chunk else ""
